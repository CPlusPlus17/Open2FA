...
class OTPAuthView extends WatchUi.View {
  ...
  function onLayout(dc as Dc) as Void {
    ...
    indicatorAngle = codeStore.size() > 10 ? 5.0 : 7.5;
  ...
  function formatCode(c as String?) as String {
    if (c == null) {
      return "";
    }
    if (c.length() == 6) {
  ...
  function drawRoundScreen(dc as Dc) as Void {
    if (!scrolling) {
      if (codeStore.isEmpty()) {
        ...
        name.setText("");
        if (noCodes != null) {
          noCodes.setText(WatchUi.loadResource(Rez.Strings.NoCodes));
        }
      } else {
        ...
        code.setText(formatCode(otpCode.getOtp().code()));
  ...
  function drawTimeStepRound(dc as Dc) as Void {
    ...
    var w = dc.getWidth();
    var h = dc.getHeight();
    var secondsLeft = codeStore.getOtpCode().getOtp().getSecondsLeft();
